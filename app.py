...
# ── Database helpers ───────────────────────────────────────────────────
def get_db():
    """Connect using pg8000 — pure Python, no system libs needed."""
    import urllib.parse
...
def create_job(job_id, customer_name, customer_email, facility_name, facility_state):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO jobs (id, customer_name, customer_email, facility_name, facility_state) VALUES (%s, %s, %s, %s, %s)",
...
def generate_report_html(job, contract_analysis, facility_synthesis, cms_data, state_data):
    ...
    if facility_synthesis.get("questions_to_ask"):
        questions_html = "".join([f"<li style='font-size:14px;color:#3d3d3a;margin-bottom:8px;line-height:1.6;font-style:italic;'>&ldquo;{q}&rdquo;</li>" for q in facility_synthesis.get("questions_to_ask", [])])
